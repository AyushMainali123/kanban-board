...
export function ColumnHeader({columnId, title}: IColumnHeaderProps) {
    ...
    return (
        <>
            {mode === "default" && (
                <button className="text-xl font-semibold cursor-pointer text-left [line-break:anywhere]" onClick={() => setMode("edit")}>{title}</button>
            )}

            {
                mode === "edit" && (
                    <form ref={formRef} id={`edit-column-title`} className=" max-w-[80%] bg-transparent" onSubmit={formMethods.handleSubmit(handleFormSubmit)}>
                        <Controller
