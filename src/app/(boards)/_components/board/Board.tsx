...
export function Board({ id, title, description, onDelete }: IBoardProps) {
    ...
    return (
        <Link 
            key={id} 
            href={`/b/${id}`}
            className="group block h-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 relative"
        >
            <Card 
                className="
            ...
            >
                <CardHeader className="flex flex-row items-center justify-between border-b border-gray-100 dark:border-gray-700">
                    <h3 className="text-xl font-bold truncate text-gray-900 dark:text-gray-50">
                        {title}
                    </h3>
                    <ArrowRight 
