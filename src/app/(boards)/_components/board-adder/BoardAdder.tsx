...
export function BoardAdder() {
    ...
    return (
        ...
        <Activity mode={mode === "default" ? "visible" : "hidden"}>
            <Button variant={"outline"} className="w-xs" onClick={() => setMode("create")}>+ Add New board</Button> 
        </Activity>
         
        <Activity mode={mode === "create" ? "visible" : "hidden"}>
            <Card className="max-w-sm">
                <CardContent>
                <form id={`create-board-form-${id}`}  onSubmit={formMethods.handleSubmit(handleFormSubmit)}>
                        <Controller
                            control={formMethods.control}
                            name="boardName"
                            render={({field, fieldState}) => (
                                <Field data-invalid={fieldState.invalid}>
                                    <FieldLabel htmlFor={field.name}>Enter board name</FieldLabel>
                                    <Input
                                     type="text"
                        ...
                        <Controller
                            ...
                            render={({field, fieldState}) => (
                                <Field data-invalid={fieldState.invalid} className="mt-6">
                                    <FieldLabel htmlFor={field.name}>Enter board Description</FieldLabel>
                                    <Textarea
                        ...
                        />
                         ...
                         <div className="flex gap-2 items-center mt-2">
                            ...
                            <Button type="button" variant={"secondary"} onClick={() => setMode("default")}>Cancel</Button>
                        </div>
                    </form>
                </CardContent>
            </Card>
